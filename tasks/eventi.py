...
async def main_menu_starters(client, message):
	...
	if len(LOOP) < 1 and CFG["miniera"]["auto"] and "⛏ Miniere di Mana (Evento) ⛰ " in kb:
	...
	if len(LOOP) < 1 and CFG["generatore"]["auto"] and "⏲ Generatore di Polvere (Evento) ♨️" in kb:
		...
		if match:
			...
			if amount == maxstorage or (amount >= CFG["generatore"]["min"] and
					curr_delta.total_seconds() % 3600 < CFG["generatore"]["maxt"]):
				LOOP.add_task(create_task("Avvia Generatore", client=client)(dust_gen))
		else:
			LOOP.add_task(create_task("Avvia Generatore", client=client)(dust_gen))
	if len(LOOP) < 1 and "💰Il Ricercato (Evento) 👺" in kb \
	and CFG["ricercato"]["auto"] and not ATTESA_ISPEZIONE.search(message.text):
	...
	and CFG["itinerario"]["auto"] and not CURRENT_MISSION_CHECK.search(message.text) \
...
async def start_wanted_hunt(client, message):
	if CFG["ricercato"]["auto"] and not LOOP.state["ispezione"]["ongoing"]:
...
async def choose_mine(client, message): # TODO make this slimmer
	if not CFG["miniera"]["auto"]:
		return
	text = ""
	kb = [ btn for sub in message.reply_markup.keyboard for btn in sub ]
	if CFG["miniera"]["lower"]:
