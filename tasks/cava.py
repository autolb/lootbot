...
def autocava_check():
	cfg = CONFIG.get()
	return (cfg["cava"]["auto"] or (cfg["imprese"]["activity"] and
...
async def riavvia_cava(client, message):
	...
	if (cfg["cava"]["auto"] or autocava_check()):
...
async def scegli_cava(client, message):
	...
	for btn in kb:
		if dest.lower() in btn.lower():
			dest = btn
			break
	@create_task(f"Scegli cava {cfg['cava']['name']}", client=client, dest=dest)
...
async def check_amuleto(client, message):
	...
	if cfg["talismani"] and TALISMAN_CHECK.search(message.text)["status"] == "❌":
...
async def gemma_la_cava(client, message): # TODO allow to retreat from exploration rather than using gems!
	...
	skip = (not dimezzato or cfg["cava"]["halvedskip"]) and cfg["cava"]["skip"] and LOOP.state["gemme"] != {} \
						and LOOP.state["gemme"] > cfg["gem-limit"]
	ritorna = "Indecisione" in LOOP.state["imprese"]["todo"] or (not dimezzato and
				(cfg["cava"]["ritorna"] or "Quanta fretta" in LOOP.state["imprese"]["todo"]) 
...
async def aggiorna_numero_gemme(client, message):
	...
	if cfg["cava"]["skip"] and curr_gem - cost > cfg["gem-limit"]:
